...
def _anthropic(prompt, sys, mid, mt):
    ...
    for attempt in range(6):  # up to 6 retries
        try:
            r = c.messages.create(
                model=mid, max_tokens=mt, system=sys,
                messages=[{"role": "user", "content": prompt}]
            )
            return r.content[0].text
        except anthropic.RateLimitError:
            wait = (2 ** attempt)  # 15, 30, 60, 120, 240, 480 sec
            print(f"  [Anthropic 429] attempt {attempt+1}/6, sleeping {wait}s...", flush=True)
            time.sleep(wait)
    
    raise RuntimeError("Anthropic rate limit: max retries exceeded")

def _mistral(prompt, sys, mid, mt):
    ...
    for attempt in range(6):
        try:
            r = c.chat.completions.create(
                model=mid, max_tokens=mt,
                messages=[{"role":"system","content":sys},{"role":"user","content":prompt}]
            )
            return r.choices[0].message.content
        except RateLimitError:
            wait = (2 ** attempt)  # 15, 30, 60, 120, 240, 480 sec
            print(f"  [Mistral 429] attempt {attempt+1}/6, sleeping {wait}s...", flush=True)
            time.sleep(wait)
    
    raise RuntimeError("Mistral rate limit: max retries exceeded")

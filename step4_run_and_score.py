...
"""
import json, re, os, csv, sys, time, argparse
...
from config import (EXPERIMENTAL_MODELS, JUDGES, API_DELAY,
                    ...
                    READABILITY_DELTA_MAX, JUDGE_POLICY)
...
def phase_score(args):
    ...
    for mf in sorted(os.listdir(out_dir)):
        ...
        # Print — exclude None from reliability (semantic pre-judge)
        ...
        for idx, (pid, pm) in enumerate(pending):
            ...
            try:
                ...
                rec = {
                    ...
                    'failures':   ['JSON_PARSE'],
                    'reason':     resp[:200] if 'resp' in dir() else ''
                }
            except Exception as e:
                errors += 1
                rec = {
